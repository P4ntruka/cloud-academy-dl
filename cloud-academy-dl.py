...
Usage:
    ...
    cloud-academy-dl [--res=<resolution>] [--cookie=<txt_file>] [--out=<output_dir>] <url>
...
def download_file(url, dest_filaneme,):
    ...
    text = 'Downloading {file:<60}'.format(file=dest_filaneme.rsplit('/',1)[1])
    with tqdm.tqdm(total=total_size, unit_scale=True, ncols=100000, desc=text, bar_format="{desc:<11} {percentage:3.0f}%s %s|%s{bar:100}{r_bar}" % ('%',Fore.LIGHTGREEN_EX, Fore.LIGHTGREEN_EX)) as progress:
